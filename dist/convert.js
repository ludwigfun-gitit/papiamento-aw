        ...
        for (const a of applyRule(base, RULES[i])) {
            ...
            for (let j = i + 1; j < n; j++) {
                for (const b of applyRule(a, RULES[j])) {
                    out.add(b);
                    for (let k = j + 1; k < n; k++) {
                        for (const c of applyRule(b, RULES[k]))
                            out.add(c);
                    }
                }
            }
...
function lookup(lower, lex) {
    ...
    if (base !== lower && lex.words.has(base))
        return base;
    for (const cand of candidates(base)) {
        if (lex.words.has(cand))
            return cand;
...
function resolveWord(lower, lex) {
...
function matchCase(original, converted) {
...
export function toAruban(text) {
    const lex = loadLexicon();
    const unknown = [];
    const seen = new Set();
    let changed = 0;
    let total = 0;
    const out = text.replace(WORD, (word, offset) => {
...
export function toCuracao(text) {
