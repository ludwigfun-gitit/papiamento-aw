...
export function googleTranslateConfigured(): boolean {
  return Boolean(process.env.GOOGLE_TRANSLATE_API_KEY)
}

export async function googleTranslate(input: GoogleTranslateInput): Promise<GoogleTranslateOutput | null> {
  ...
  const parts: { field: 'title' | 'body'; text: string }[] = []
  if (input.title) parts.push({ field: 'title', text: input.title })
  if (input.body) parts.push({ field: 'body', text: input.body })
  ...
  if (!resp.ok) {
    ...
    if (resp.status === 400 && /language|target|source/i.test(text)) {
      console.error(
        `[google-translate] the API rejected the language pair ${source}→${target}: ${text.slice(0, 200)}. ` +
  ...
  const translations: any[] = json?.data?.translations ?? []
  if (translations.length !== parts.length) throw new Error('google translate: unexpected response shape')

  const out: GoogleTranslateOutput = {
